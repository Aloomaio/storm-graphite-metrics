			...
			for (DataPoint p : dataPoints) {
				...
				if(p.value instanceof Map) {
					...
					for(Map.Entry e : entries) {
						LOG.trace(String.format("Registering a value inside a datapoint map to graphite: %s, %s. Value type is: %s", e.getKey(), e.getValue(), e.getKey(), e.getValue().getClass().getCanonicalName()));
